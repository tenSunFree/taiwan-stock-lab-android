...
The current implementation focuses on a production-oriented data foundation built with Kotlin, Clean Architecture, a multi-module Gradle setup, structured concurrency, and an offline-first Room 3 persistence layer.
...
- Hilt dependency-injection composition root
...
- **`:app`** — Android application entry point. Owns application-level composition and navigation. Currently contains the initial Activity shell.
...
| Dependency injection | Hilt composition root | ⏳ Next |
| Presentation state | ViewModel + MVI-style UDF | ⏳ Planned |
